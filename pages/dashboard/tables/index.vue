  ...
  <div>
    ...
    <template v-if="!selectedEventId">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="font-display text-2xl font-bold text-white mb-1">Mesas</h2>
          <p class="text-white/45 text-sm">Selecciona un evento para organizar sus mesas</p>
        </div>
        <div class="relative">
      ...
      <div v-else class="text-center py-20">
        ...
        <p class="text-white/40 text-sm mb-6">Crea un evento para comenzar a organizar mesas.</p>
    ...
    <template v-else>
      ...
      <div v-if="loadingTables" class="grid sm:grid-cols-3 lg:grid-cols-4 gap-4">
        <div v-for="i in 8" :key="i" class="h-40 shimmer rounded-xl" />
      </div>
...
async function selectEvent(id: string) {
  ...
  try {
    const res = await get<{ data: EventTable[] }>('/api/tables', { eventId: id })
